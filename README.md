...
包裹留证是一款面向电商商家和打包工作台的 Android 录像工具。手机固定在打包台后，点一次“开始工作”，应用便会持续录像、自动识别面单条码，并把识别时刻写入录像时间线。发生售后争议时，可按快递单号快速找到对应画面。
...
- **自动识别面单**：支持常见一维物流条码，识别后自动记录快递单号和录像时间点
...
```powershell
git tag v0.5.8+11008
pwsh -NoProfile -File Tools\Publish-Android.ps1 `
  -SigningDirectory <仓库外的签名目录>
```

正式发布脚本要求当前提交已有版本标签且工作区干净。推荐标签使用 `v<版本名>+<递增 versionCode>` 格式；例如 `v0.5.8+11008` 会生成版本 `0.5.8`、版本号 `11008`。签名目录中需包含密钥文件及 UTF-8 编码的 `签名凭据.txt`，目录必须位于仓库外。
